...
file_handler = logging.FileHandler('coinbase.log')
...
def get_historical_data(product, start, end, granularity):
    ...
    if response.status_code == 200:
        ...
        logger.info(f"Retrieved data for {product} - {start} to {end}")
        return df
    else:
        logger.error(f"Failed to retrieve data for {product} - {start} to {end}")
        return None
